...
class AbstractWars {
 public:
  ...
  // ----------------------------------------------
  vector<int> others;
  // picks a random base to attack based on distance to the opponent bases: the
  // closer the base, the higher the chances are
  int getRandomBase(int sourceInd) {
    vector<double> probs(others.size());
    double sp = 0;
    for (int i = 0; i < (int)others.size(); ++i) {
      int ind = others[i];
      probs[i] = 1 / (pow(baseX[sourceInd] - baseX[ind], 2) +
                      pow(baseY[sourceInd] - baseY[ind], 2));
      sp += probs[i];
    }
  ...
  vector<int> sendTroops(vector<int> bases, vector<int> troops) {
    ...
    others.resize(0);
    for (int i = 0; i < B; ++i)
      if (bases[2 * i] != 0) others.push_back(i);
    if (others.size() == 0) {
      // noone to fight!
      return vector<int>(0);
    }
